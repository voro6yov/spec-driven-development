...
description: "Scaffolds the command-side aggregate package (command repository module only) from a command-repo-spec file and a target-locations-finder report. Emits the repository class stub with the matching spec block embedded as a docstring and wires inter-module imports where they can be inferred mechanically. Mapper modules are owned by `@mappers-scaffolder`. Table modules are owned by `@table-scaffolder`. Invoke with: @command-repo-files-scaffolder <command_spec_file> <locations_report_text>"
...
2. **Aggregator `__init__.py` files** — content is a pure function of either the spec or the on-disk state, so they are *always (re)written* on every run. This applies to: the per-aggregate `__init__.py` this agent creates, and the parent `<repo_dir>/__init__.py` (refreshed by listing immediate subpackages on disk). Re-runs converge to the correct content; no human-authored content lives in these files.

This agent owns the mechanical, per-aggregate file scaffolding for the repository package only. Mapper modules and `mappers/__init__.py` are owned by `@mappers-scaffolder` — invoke it separately to scaffold the `mappers/` sub-package. Table modules and the `tables/` aggregators are handled by `@table-scaffolder`. Context-integration concerns (unit_of_work copy + `containers.py` wiring) are handled by `@unit-of-work-scaffolder`. Migrations are handled by downstream implementers.
...
**Existence-check rule for stub files.** Before every `Write` of the *stub* file (repository module), run `test -f <path>` via Bash and only `Write` when the file does not exist. *Aggregator* `__init__.py` files (per-aggregate `__init__.py`, parent `<repo_dir>/__init__.py`) skip this check — they are always written/rewritten with the spec- or disk-derived content. The `Write` tool itself overwrites unconditionally, so the existence check is the *only* idempotence guard for stubs.
...
- The `mappers/` sub-package and its `__init__.py` are *not* written here — `@mappers-scaffolder` owns them and must be invoked separately.
...
Emit a bare bullet list with the single absolute path to the repository stub module — one bullet, nothing else on the line. Include the stub regardless of whether this run wrote it or it already existed; the next agent uses the list as its worklist. Do **not** include `__init__.py` files, mapper modules, headers, status markers, class names, or any other commentary. (Mapper stub paths are emitted by `@mappers-scaffolder`.)
