...
3. **Framework-managed expansions** driven by the mapper choice in Section 2 — apply only when the corresponding domain field actually exists on the diagram:
   - **`Full Aggregate Mapper`, `Aggregate Mapper with Children`, or `Child Entity Mapper`** with a domain field named `status` whose `<<Value Object>>` type matches the **framework `Status` VO** — recognised by either (a) the field's type literally being `Status`, **or** (b) the referenced VO class declaring both a `status` field (typed `str`) and an `error` field (typed `Optional[str]` / `str | None`) → replace the would-be JSONB `status` column with two columns: `status: String NOT NULL` and `status_error: String NULL`. This matches the Full / Child Entity mapper templates, which write `aggregate.status.status` and `aggregate.status.error` into separate columns rather than a JSONB blob. If the field is absent or its VO does not match the framework `Status` shape, emit no `status` columns; a non-framework `status` VO falls through to a single JSONB column per the regular sub-step 2 rule.
...
1. **`### Entity Relationship`** — a `classDiagram` with one `<<Table>>` class per table from Section 2, named in PascalCase as `\{Aggregate\}Table` / `\{Child\}Table`. List **every column from the matching `### Table:` block, in the same order** — including JSONB columns. The ERD is a faithful, deterministic projection of the schema; do not selectively omit columns. Render each column as `-<column_name>: <Type>` on its own line (no constraints, no description) using the SQLAlchemy type token from the `### Table:` row verbatim. For aggregates with children, draw `\{Aggregate\}Table "1" --* "0..n" \{Child\}Table : owns`. Wrap the diagram in a fenced `mermaid` block whose body opens with a YAML frontmatter delimited on both sides by `---` lines (carrying `title: \{Context\} Storage Model` using the context name resolved in Step 1, plus a `config:` mapping with `class: hideEmptyMembersBox: true`), followed by a blank line and then `classDiagram` — exactly matching the wrapper in `command-repo-spec-template`.
