...
- **Projection-column propagation (table → repository).** A column added to or removed from a mapped table makes the command repository stale: the repository projects every table through an explicit `select(*self.<x>_columns)` list (both variants — `Simple Command Repository` and `Command Repository with Children` — project explicitly; neither uses `select(table)`), so a column the table now has is never selected. Once §4.3 updates the owning mapper's `from_row` / `from_rows` to read that column, the load raises `NoSuchColumnError` against the unselected key — the exact failure this propagation prevents. (It is the repository-side twin of §4.3's mapper propagation: the projection list is a *third* consumer of the table's column set, parallel to the table def and the mapper.) For each table `T` in §4.2's `### Modified` set carrying at least one `Columns added` or `Columns removed` sub-bullet (a `Columns altered` type change or a `Nullability flipped` leaves the projected column *names* unchanged, so neither propagates here), emit (or merge into) the Repository Changes `Projection columns changed:` bullet:
  - `` `<T>` — `<col>` added; `<col>` removed `` — built from `T`'s §4.2 `Columns added` / `Columns removed` sub-bullets, columns in the order they appear there, one sub-bullet line per modified table. Omit clauses with no entries.

  No table↔repository join is needed: §2.Tables lists only this aggregate's tables, and the single command repository projects all of them — every modified table is in scope. Which `<x>_columns` property to patch is resolved at apply time (the code updater locates the property by its `<T>_table.c.*` references), so this report only names the table and its added/removed columns. Do **not** read the repository module here to confirm an explicit projection exists — that on-disk check is the code updater's job; a repository that reads via whole-table `select(<T>_table)` simply makes the propagated row a no-op downstream.

If the section has no pattern flip, no lookup changes, and no projection-column propagation, the body is `_no changes_`.
...
3. Compute derived fields per row:
   ...
   - **`source_delta`** — best-effort lookup against `<dir>/<stem>.domain/updates.md`:
     ...
     - When a match is found, derive the `source_delta` string as `<category>: <ClassName> <delta_phrase>` where `<category>` is the affected category from the domain footer (or inferred from the class's stereotype per `spec-core:update-reports` (domain schema)'s mapping), and `<delta_phrase>` is a short verbatim summary (`attribute email added`, `attribute legacy_field removed`, `finder user_of_email added`, etc.).
