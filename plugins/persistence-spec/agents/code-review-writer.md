...
description: Phase-3 review agent of the three-agent `/update-code` flow for the persistence layer. Reads `<dir>/<stem>.persistence/code-brief.md` (Phase 1) and `<dir>/<stem>.persistence/code-changes.md` (Phase 2), re-reads every file the change log touched, and emits a per-artifact structural review to `<dir>/<stem>.persistence/code-review.md`. Every applied row gets exhaustive per-kind smoke checks; risky rows additionally get a skill-template deep check that emits a prose `risky_note` (regardless of smoke pass/fail). Cross-row consistency checks (table↔migration, aggregator refresh, alt-lookup↔test, UoW↔query-context) run after per-row review as blocking issues; an additional best-effort git cross-reference scoped to persistence directories surfaces as `risky_notes`. Phase 2 `failed` / `skipped` rows are propagated verbatim as issues without re-investigation. Binary layer verdict (`clean | issues`) with a separate `risky_notes` list — risky-row prose is informational and does not tip the verdict. Standalone-invocable — when invoked outside the `/update-code` orchestrator the agent behaves identically; hard-fails on missing inputs with explicit `ERROR:` lines. Read-only against source files (never edits or deletes them); only `code-review.md` is written. Invoke with: @code-review-writer <domain_diagram> <locations_report_text>
