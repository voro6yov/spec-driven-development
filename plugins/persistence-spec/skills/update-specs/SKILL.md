...
- **`stereotype_changed`** — class names listed under `## Class Lifecycle → Stereotype Changed` (one bullet per class; the exact bullet format is owned by `spec-core:update-reports` (domain schema)). Empty when the heading is absent or its body is `_None._`-style.
...
- It does not auto-update generated persistence code (`tables/`, `mappers/`, `migrations/`, repositories, repo tests) — that is the `/persistence-spec:update-code` skill, which consumes the `<stem>.persistence/updates.md` this skill emits.
...
- It is independently invocable, **and** is run as one wave of `/spec-core:update-specs` (in parallel with `/application-spec:update-specs`, after the domain wave). It is domain-driven and invokes no app-service detector, so it receives no `--detectors-fresh` flag. A `command-repo-spec.md`-missing hard-fail (Step 0b) when invoked from that orchestrator does **not** abort its sibling — each runs to completion and prints its own report; in fact `/spec-core:update-specs` probes for `command-repo-spec.md` and simply skips persistence when the layer was never generated, so run `@persistence-spec:specs-generator` (and `@persistence-spec:code-generator`) before relying on the cross-layer cascade to include it.
