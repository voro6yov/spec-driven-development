---
name: update-specs
description: Surgically updates the command repository spec after a domain diagram change — regenerates the snapshot sections from the current diagram, appends delta-driven migration rows to §2.Migrations, and emits the persistence updates report. Consumes the domain updates.md; never re-diffs the diagram. Invoke with: /persistence-spec:update-specs <domain_diagram>
...
- It is independently invocable, **and** is chained as **Step 10** of domain `/update-specs` — the first of the four downstream chain steps, before `/application-spec:update-specs`. A `command-repo-spec.md`-missing hard-fail (Step 0b) when invoked from that chain aborts the rest of the cascade; run `/persistence-spec:generate-specs` (and `/persistence-spec:generate-code`) before relying on the domain-level cascade.
