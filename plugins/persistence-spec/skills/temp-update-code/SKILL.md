...
The orchestrator itself **never reads a spec sibling and never edits source code** — those responsibilities live entirely in the `@persistence-spec:code-brief-writer`, `@persistence-spec:code-change-writer`, and `@persistence-spec:code-review-writer` agents.

## Inputs

Given `<domain_diagram>` at `<dir>/<stem>.md`, the orchestrator reads only `<dir>/<stem>.persistence/updates.md` for the preflight gates. The brief, implement, and review agents own every other read.
...
| 2 (implement) | `<dir>/<stem>.persistence/code-changes.md` + source edits | `@persistence-spec:code-change-writer` |
...
Parse `<dir>/<stem>.persistence/updates.md` and check whether all body sections read `_no changes_` AND its `## Affected Artifacts` table is empty or absent. (Refer to `persistence-spec:updates-report-template` for the exact body markers.) If so, print:
...
Artifacts:
- <dir>/<stem>.persistence/code-brief.md   (Phase 1)
- <dir>/<stem>.persistence/code-changes.md (Phase 2)
...
- It does not detect hand-edited source on disk. Operators with hand-tuned tables, mappers, or repository methods must reconcile manually after Phase 2.
