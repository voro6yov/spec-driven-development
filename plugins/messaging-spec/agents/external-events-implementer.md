...
For each `<CommandClass>`, derive `<command_aggregate_snake>` — the snake_case name of *this service's* local aggregate whose state responds to the event, used as the "local aggregate" prefix for type-resolution disambiguation in Step 7c:

- **`Commands` handler** (`<CommandClass>` ends in `Commands`, e.g. `ProfileCommands`) → strip the trailing `Commands` suffix and apply the PascalCase → snake_case rule (defined in Step 7c), e.g. `profile`.
- **Ops handler** (`<CommandClass>` is a free-form ops service class with no `Commands` suffix, e.g. `SubjectTagging`) → the ops service's bound aggregate is `<stem>` (recovered from `<commands_diagram>` in Step 3, the aggregate this whole service centers on), so set `<command_aggregate_snake>` = snake_case(`<stem>`). Do **not** derive it from the ops class name — the ops class is a free-form noun phrase unrelated to the aggregate package. (For a `Commands` handler this is equivalent, since `<stem>` is the snake_case of the aggregate whose PascalCase + `Commands` is `<CommandClass>`.)

**Do not** use Source Destination for this purpose: Source Destination names the *publishing* service's aggregate (which has no package in this service's domain), whereas the rule above resolves the local aggregate (which does).
