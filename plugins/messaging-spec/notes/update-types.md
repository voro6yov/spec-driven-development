...
Tier 2 is the only "real work" tier, and it is unusually contained: re-run one agent (`event-fields-writer`) for the subset of consumers whose `internal` subscriptions intersect the changed-domain-event set. This updater runs as **Step 13 of domain `/update-specs`** (wired, domain-spec ≥ 0.29.0) — the last of the persistence (10) / application (11) / rest-api (12) / messaging (13) chain, invoked unconditionally; the skill's own Step 0c "no consumer specs" path keeps it a graceful no-op when there is no messaging layer, so it never aborts the cascade. See the as-built "Chaining contract" in `persistence-spec/notes/spec-updater-approaches.md`. The chained step covers only the domain-driven (Tier 2 + the Tier-1 hard-fails) axis above — the commands-diagram axis remains a separate trigger (a `messaging-spec:updates-detector` invocation, or a fresh `/messaging-spec:generate-code` run per consumer).
