...
For the follow-on integration that wires the application-spec commands-diagram detector report into this skill, see [`commands-queries-integration-approach.md`](commands-queries-integration-approach.md) — the "no commands-diagram detector" cut documented below has since been superseded by that note.

The chosen approach is **a thin, two-axis dispatcher** — *not* a surgical splicer. The reasoning, the pipeline, and the cross-plugin integration that wires the commands-diagram detector axis in are below.
...
4. **Commands-diagram-axis integration (no new messaging-spec detector).** Done via [`commands-queries-integration-approach.md`](commands-queries-integration-approach.md): `/messaging-spec:update-specs` invokes the shared `application-spec:commands-updates-detector` at Step 0d and unions its dispatch signals with the domain axis (Step 4). The previously-planned plugin-local `messaging-spec:updates-detector` is superseded — the shared application-spec detector emits the `messaging-markers` / `external-domain-events` / `methods` categories messaging needs, and three plugin-local copies of the same parsing logic would only drift.
