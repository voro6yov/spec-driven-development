...
1. Verify presence with `Bash`:
   ...
   let this skill produce the report itself, or run `/spec-core:update-specs <domain_diagram>`
   (which runs the application wave first, then this skill with the flag) — or
   `/application-spec:update-specs <domain_diagram>` to produce it.
   ```
...
**Domain axis** (from `<stem>.domain/updates.md`; the exact bullet formats are owned by `spec-core:update-reports` (domain schema)):
...
**Commands-diagram axis** (from `<stem>.application/commands-updates.md`; the exact bullet formats are owned by `spec-core:update-reports` (application-axis schema)):
