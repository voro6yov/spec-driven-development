...
#### 4b. Patterns lookup (hardcoded role→patterns table)
...
- Kind: <kind>
- Risk: <risk>
- Patterns: <pattern1>, <pattern2>, ...  _or_  `(none — regen owned by @<agent>)`
- Members:
...
- It does not load any pattern template body. The role→patterns mapping is hardcoded; pattern *names* go into the brief, and Phase 2 Reads their bodies from the `application-spec:patterns` umbrella.
