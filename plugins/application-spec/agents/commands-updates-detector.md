...
2. **The anchor class** — the unique class with stereotype `<<Application>>`. Record its name, dependencies (private attributes), and public methods.
   ...
   - **Anchor name comparison** — when both versions have exactly one anchor and the names differ → hard-fail (`ERROR: anchor class renamed in <commands_diagram> (<HEAD_name> → <WT_name>); aggregate-root rename is a multi-file rename. Route to @application-spec:specs-generator.`), write nothing.

3. **Stereotype-change gate** — for every class present in both versions whose stereotype string differs → hard-fail (`ERROR: stereotype changed on class <X> in <commands_diagram> (<<HEAD_stereo>> → <<WT_stereo>>); cross-category move. Route to @application-spec:specs-generator.`), write nothing. This gate applies to anchor and non-anchor classes alike. Empty-string stereotypes on non-anchor classes are *not* an error in themselves — they only error if they differ between versions.
...
| 2 | `<commands_diagram>` missing or unreadable | `ERROR: <commands_diagram> not found or unreadable.` | Run `@application-spec:specs-generator <domain_diagram>` first. |
| 3 | `<commands_diagram>` is untracked, or absent from HEAD (first-run) | `ERROR: <commands_diagram> is untracked / not in HEAD. /application-spec:update-specs is not the first-run pipeline; run @application-spec:specs-generator <domain_diagram> first.` | Run `@application-spec:specs-generator`. |
...
| 9 | Anchor class renamed (different name in HEAD vs. working tree, both versions having exactly one anchor) | `ERROR: anchor class renamed in <commands_diagram> (<HEAD_name> → <WT_name>); aggregate-root rename is a multi-file rename. Route to @application-spec:specs-generator.` | Run `@application-spec:specs-generator`. |
| 10 | Any class's stereotype string changed between HEAD and working tree (anchor or non-anchor) | `ERROR: stereotype changed on class <X> in <commands_diagram> (<<HEAD_stereo>> → <<WT_stereo>>); cross-category move. Route to @application-spec:specs-generator.` | Run `@application-spec:specs-generator`. |
