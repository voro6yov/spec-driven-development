...
- **Anchor methods** — under `## Methods` (or the per-method blocks the detector emits per `spec-core:update-reports` (application-axis schema)), walk each `### Method:` heading. Bind `commands.methods = {method_signature: (bucket, sub_sections_changed)}` where bucket ∈ `{added, removed, modified}` and sub_sections_changed is the bullet list under `Sub-sections changed:` (empty for added/removed).
