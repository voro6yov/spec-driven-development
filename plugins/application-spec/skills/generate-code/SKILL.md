...
In a single message, invoke one `application-spec:ops-implementer` agent per `<op-name>` in `<ops_services>`. Each invocation uses the prompt:

```
$ARGUMENTS[0] <locations_report_text> <op-name>
```
...
- One `application-spec:ops-tests-implementer` per `<op-name>` in `<ops_services>`, each with prompt `$ARGUMENTS[0] <tests_dir> <op-name>`. Omit these entirely if `<ops_services>` (Step 1) is empty.
