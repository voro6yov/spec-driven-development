...
- `<category>` is one of the categories from the matched axis's `## Affected Categories` footer vocabulary:
  ...
  - **App-service axes** (per `spec-core:update-reports` (application-axis schema)): `methods`, `dependencies`, `raised-exceptions`, `external-interfaces`. (`surface-markers` and `messaging-markers` never appear on this report — they're owned by REST API and messaging axes respectively.)
