...
- A prose section belongs to a class block when its heading parses to that class name. Resolution applies two passes in order: (1) **strict forms** — `ClassName`, `ClassName.method_name`, or `ClassName.method_name(...)`; (2) **topic-suffixed fallback** — if no strict match, split the heading at the first occurrence of ` — ` (em-dash with surrounding spaces), ` – ` (en-dash with surrounding spaces), ` - ` (hyphen with surrounding spaces), or `: ` (colon followed by a space), and retry the prefix against the strict forms. The trailing topic text is descriptive and does not need to resolve. The full original heading text — including any topic suffix — is preserved verbatim in the **Prose — `<heading>`** sub-section label regardless of which form matched.
