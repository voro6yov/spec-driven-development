...
description: "Domain-only variant of `update-specs` for testing. Detects diagram deltas, regenerates only affected categories, splices into the live spec, refreshes exceptions, and conditionally replans tests — without cascading to downstream spec layers. Invoke with: /temp-update-specs <domain_diagram>"
argument-hint: <domain_diagram>
allowed-tools: Read, Bash, Agent, Skill
---

You are a domain-only DDD spec **update** orchestrator. This is a testing variant of `/update-specs` that runs the same surgical domain-side flow but **never cascades** to `/persistence-spec:update-specs`, `/application-spec:update-specs`, `/rest-api-spec:update-specs`, or `/messaging-spec:update-specs`. Use it to isolate domain-side behavior when iterating on the domain pipeline.

Given a diagram whose working-tree differs from `git HEAD`, regenerate only the affected slices of the existing sibling spec artifacts in-place — do not rerun the full `/generate-specs` pipeline, do not touch class blocks unrelated to the change, and do not ask for confirmation before writing.
...
This skill writes nothing under `<stem>.persistence/`, `<stem>.application/`, `<stem>.rest-api/`, or `<stem>.messaging/`. Downstream spec layers are not touched.
...
specs from scratch, or fix the HEAD revision before retrying `/temp-update-specs`.
```
...
Do not emit additional commentary. Each invoked agent already prints its own per-step report. The workflow is complete after this line — no cascade follows.
...
- The orchestrator does not roll back partial writes. **Re-running `/temp-update-specs` after fixing the trigger is the supported recovery path** — every step is idempotent on stable inputs:
...
- The only failures `/temp-update-specs` cannot retry through are degraded-baseline (1a) and stereotype-change (1b) conditions — both gates will fire again on re-run. The error messages explicitly direct the operator to `/generate-specs <domain_diagram>` for those cases.

## What this skill deliberately does not do

- It does not cascade to `/persistence-spec:update-specs`, `/application-spec:update-specs`, `/rest-api-spec:update-specs`, or `/messaging-spec:update-specs`. Use `/update-specs` when you want the full cross-layer fan-out; use this skill when isolating domain-side behavior for testing.
- It does not invoke `application-spec:commands-updates-detector` or `application-spec:queries-updates-detector`. No `<stem>.application/{commands,queries}-updates.md` is written by this skill.
...
- It does not auto-update generated code or test bodies — pair this skill with `/temp-update-code <domain_diagram>` for the domain-only code-update analog.
