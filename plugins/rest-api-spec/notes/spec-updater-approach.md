...
The updater handles **only the domain-driven axis**. Changes that originate in `<stem>.commands.md` / `<stem>.queries.md` (the application-service diagrams) — endpoint added/removed, method signature changed, a surface added/removed/renamed — are out of scope here; see *What this updater does NOT cover*. Per `update-types.md`, the domain diagram's contribution to the REST spec is so narrow that the median domain change leaves `spec.md` byte-identical and the updater early-exits.
...
- **Commands/queries-diagram changes are out of scope — and they are the dominant axis.** Most REST-spec changes in practice — an endpoint added/removed (Tables 2/3), a method signature changed (parameters, return type → Tables 4/5/6), a surface added/removed/renamed (Table 1's Surfaces row + the `## Surface:` section set + orphaned sections), the resource's plural changed (Table 1) — originate in the *application-service* diagrams (`<stem>.commands.md`, `<stem>.queries.md`) or in Table 1 itself, and `<stem>.domain/updates.md` does not capture them. A complete rest-api-spec updater needs an updates-detector analog that diffs those two diagrams — the **same two diagrams** an `application-spec:updates-detector` would diff, so a single shared detector could feed both the application updater and the rest-api updater — **or** must accept that those changes are handled by re-running `/rest-api-spec:generate-specs`. This is *the* central design decision for the updater; the domain-driven axis cataloged here and handled by this skill is the narrow minority. When the shared detector lands, its report wires into this updater's dispatch by marking `response_fields_dirty` / `request_fields_dirty` / `parameter_mapping_dirty` (and, for surface / endpoint changes, by additionally re-running `endpoint-tables-writer` and reconciling Table 1's Surfaces row).
...
The persistence-spec design (`spec-updater-approaches.md`) and the application-spec design (`spec-updater-approach.md`) both anticipate this updater as **Step 11 of domain `/update-specs`** — opt-in by file presence (`<stem>.rest-api/spec.md` exists), reading the same `<stem>.domain/updates.md`. The chained step covers only the domain-driven axis; the commands/queries-diagram axis remains a separate trigger (the shared `application-spec:updates-detector` invocation discussed above, or a fresh `/rest-api-spec:generate-specs` run).
