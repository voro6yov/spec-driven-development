...
- Initialize the current surface set to `{v1}` at the start of the commands class body.
...
- Composite-key detection (Step 2) ignores `on_*` message-handler methods — adding a handler with an aggregate-id-shaped parameter must not reroute a composite-key aggregate's key fields from the query string into the request body.
