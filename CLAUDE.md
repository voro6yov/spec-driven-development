...
- **Pattern docs live under the `patterns` umbrella skill (domain-spec):** reference docs are supporting files at `skills/patterns/<name>/index.md` (+ `template.md`/`examples.md`), not standalone skills. Domain-spec consumers frontmatter-load `domain-spec:patterns` and Read `<patterns_dir>/<name>/index.md`; never re-register a pattern as its own skill. Pattern identifiers in specs/briefs keep the `domain-spec:<name>` token form. Eight cross-plugin-consumed refs (`aggregate-fixtures`, `collection-value-objects`, `constructor-guard-type-mapping`, `domain-exceptions`, `flat-constructor-arguments`, `optional-values`, `package-layout`, `updates-report-template`) are **dual-homed** until Wave 3: the standalone skill stays registered (authoritative — foreign plugins load it); its umbrella copy must be re-synced byte-identically after any edit.
